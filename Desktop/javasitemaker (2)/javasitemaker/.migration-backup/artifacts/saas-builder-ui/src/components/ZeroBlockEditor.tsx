  ...
  if (el.type === "video") {
    ...
    return (
      <div style={{ ...baseStyle, background: "#000", display: "flex", alignItems: "center", justifyContent: "center",
        border: "2px dashed rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.3)", fontSize: 12 }}>
        ▶ Видео
      </div>
    );
  }
  ...
  if (el.type === "group") {
    return (
      <div style={{ ...baseStyle, background: s.bg && s.bg !== "transparent" ? s.bg : "rgba(124,58,237,0.04)",
        ...
        <span style={{ color: "rgba(124,58,237,0.5)", fontSize: 11 }}>⊞ Группа ({(c.childIds || []).length} эл.)</span>
      </div>
    );
  }
...
function PropsPanel({ el, onChange, onDelete, onDuplicate, bp }:
  { el: ZeroElement; onChange: (patch: Partial<ZeroElement>) => void; onDelete: () => void; onDuplicate: () => void; bp: Breakpoint }) {
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border flex-shrink-0">
        ...
        <div className="flex gap-1">
          <button onClick={onDuplicate} title="Дублировать"
            className="w-6 h-6 flex items-center justify-center rounded text-muted-foreground hover:text-foreground hover:bg-secondary transition text-xs">⧉</button>
          <button onClick={onDelete} title="Удалить"
            className="w-6 h-6 flex items-center justify-center rounded text-muted-foreground hover:text-red-400 hover:bg-red-500/10 transition text-xs">✕</button>
        </div>
      </div>
        ...
        {tab === "content" && (
          <>
            {el.type === "text" && (
              <>
                <div>
                  <label className="text-[10px] text-muted-foreground font-medium block mb-0.5">Текст</label>
                  <textarea value={el.content.text || ""} onChange={e => upContent({ text: e.target.value })} rows={5}
                    className="w-full bg-secondary border border-border rounded-lg px-2 py-1.5 text-xs text-foreground resize-none focus:outline-none focus:border-primary" />
                </div>
              </>
            )}
            {el.type === "image" && (
              <>
                {inp("URL изображения", el.content.src, v => upContent({ src: v }), "text", "https://")}
                {inp("Alt текст", el.content.alt, v => upContent({ alt: v }), "text")}
              </>
            )}
            {el.type === "button" && (
              <>
                {inp("Текст кнопки", el.content.label, v => upContent({ label: v }), "text")}
                {inp("Ссылка (href)", el.content.href, v => upContent({ href: v }), "text", "https:// или #anchor")}
                <div>
                  <label className="text-[10px] text-muted-foreground font-medium block mb-0.5">Открыть в</label>
            ...
            {el.type === "html" && (
              <div>
                <label className="text-[10px] text-muted-foreground font-medium block mb-0.5">HTML-код</label>
                <textarea value={el.content.html || ""} onChange={e => upContent({ html: e.target.value })} rows={10}
                  className="w-full bg-secondary border border-border rounded-lg px-2 py-1.5 text-xs text-foreground font-mono resize-none focus:outline-none focus:border-primary"
                  placeholder="<div style='color:white'>...</div>" />
              </div>
            )}
            {el.type === "video" && inp("URL видео (YouTube/Vimeo)", el.content.videoUrl, v => upContent({ videoUrl: v }), "text", "https://youtube.com/watch?v=...")}
            {el.type === "form" && (
                ...
                {(el.content.formFields || []).map((f, i) => (
                  <div key={f.id} className="bg-secondary/50 rounded-lg p-2 space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      }} className="text-red-400/60 hover:text-red-400 text-[10px]">✕</button>
                    </div>
              ...
              <>
                ...
                <div className="relative bg-secondary/40 rounded-lg overflow-hidden border border-border/50 p-2">
                  ...
                  <button onClick={() => {
                    ...
                    {animPreview ? "▶ воспроизводится..." : "▷ Предпросмотр анимации"}
                  </button>
                </div>
              </>
            )}
...
const TYPE_ICONS: Record<ZeroElementType, string> = {
  text: "T", image: "🖼", button: "□", shape: "◆", html: "</>", video: "▶", form: "☑",
  path: "✏", group: "⊞",
};

const BP_ICONS: Record<Breakpoint, string> = { desktop: "🖥", tablet: "📱", mobile: "📲" };
...
export default function ZeroBlockEditor({ data, onChange, onClose, blockName = "Zero Block" }: ZeroBlockEditorProps) {
      ...
      <div className="bg-card border-b border-border flex-shrink-0 z-10 overflow-x-auto">
        <div className="flex items-center gap-2 px-3 py-2.5 min-w-max">
          ...
          <div className="flex items-center gap-1.5 ml-auto flex-shrink-0">
            ...
            {saved && <span className="text-[11px] text-emerald-400 hidden sm:flex items-center gap-1">✓ Сохранено</span>}
            <button onClick={() => { onChange(buildData()); onClose(); }}
            ...
            <div>
              <div className="px-2.5 py-2 text-[10px] text-muted-foreground font-bold uppercase tracking-wider flex items-center justify-between">
                ...
                {selectedIds.length > 0 && (
                  <button onClick={deleteSelected} className="text-red-400/60 hover:text-red-400 text-[10px]">✕</button>
                )}
              </div>
                  ...
                  return (
                    <div key={el.id} onClick={() => setSelectedIds([el.id])}
                      ...
                      <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 flex-shrink-0">
                        <button onClick={e => { e.stopPropagation(); moveLayer(el.id, "up"); }} title="Выше" className="hover:text-foreground">↑</button>
                        <button onClick={e => { e.stopPropagation(); moveLayer(el.id, "down"); }} title="Ниже" className="hover:text-foreground">↓</button>
          ...
          <div style={{ transform: `scale(${zoom / 100})`, transformOrigin: "top center" }}>
            ...
            {drawingPathId && (
              <div className="text-center py-1 mb-2 text-[11px] text-primary font-semibold bg-primary/10 border border-primary/20 rounded-lg">
                ✏ Режим рисования — клик = прямой сегмент &nbsp;·&nbsp; <span className="text-emerald-400">клик+тяни = безье</span> &nbsp;·&nbsp;
            ...
            {editPointsPath && (
              <div className="text-center py-1 mb-2 text-[11px] text-emerald-400 font-semibold bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                ⬡ Редактор точек — тяни якоря (синий/фиолет.) &nbsp;·&nbsp; тяни ручки (зел./жёлт.) для безье &nbsp;·&nbsp;
              ...
              {sorted.map(el => {
                ...
                return (
                  <div key={el.id}
                    ...
                    {/* Lock badge */}
                    {el.locked && (
                      <div className="absolute top-0.5 right-0.5 text-[10px] bg-amber-500/20 text-amber-400 rounded px-1">🔒</div>
                    )}
            ...
            <div className="border-t border-border p-2 space-y-1.5 flex-shrink-0">
              ...
              {selectedEl?.type === "path" && (<>
                <button
                  ...
                  {drawingPathId === selectedEl.id ? "⏹ Завершить путь" : "✏ Рисовать путь"}
                </button>
                <button
                  onClick={() => {
                  ...
                  {editPointsPath === selectedEl.id ? "✓ Выйти из редактора точек" : "⬡ Редактировать точки (Безье)"}
                </button>
              </>)}
              ...
              {selectedEl?.type === "text" && (
                <button onClick={() => setEditingTextId(editingTextId === selectedEl.id ? null : selectedEl.id)}
                  ...
                  {editingTextId === selectedEl.id ? "✓ Выйти из редактора текста" : "T Редактировать текст (2× клик)"}
                </button>
              )}
              {/* Alignment buttons */}
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-3 gap-1">
                  {([
                    ["left", "⬡◁", "По левому краю"],
                    ["center-h", "↔", "По центру (гориз.)"],
                    ["right", "▷⬡", "По правому краю"],
                    ["top", "△⬡", "По верхнему краю"],
                    ["center-v", "↕", "По центру (верт.)"],
                    ["bottom", "⬡▽", "По нижнему краю"],
                  ] as const).map(([axis, icon, title]) => (
                    <button key={axis} onClick={() => alignElements(axis)} title={title}
                      className="py-1.5 bg-secondary hover:bg-primary/15 hover:text-primary rounded-lg text-[11px] text-muted-foreground transition font-mono">
                      {icon}
                    </button>
                  ))}
                </div>
              ...
              {selectedIds.length >= 2 && (
                <button onClick={groupSelected} title="Ctrl+G"
                  className="w-full py-1.5 bg-violet-500/10 hover:bg-violet-500/20 rounded-lg text-[11px] text-violet-400 transition font-semibold">
                  ⊞ Сгруппировать (Ctrl+G)
                </button>
              )}
              <div className="flex gap-1">
                <button onClick={duplicateSelected} title="Ctrl+D"
                  className="flex-1 py-1.5 bg-secondary hover:bg-secondary/80 rounded-lg text-[11px] text-muted-foreground hover:text-foreground transition">
                  ⧉ Копия
                </button>
                <button onClick={deleteSelected} title="Delete"
                  className="flex-1 py-1.5 bg-red-500/10 hover:bg-red-500/20 rounded-lg text-[11px] text-red-400 transition">
                  ✕ Удалить
                </button>
              </div>
            </div>
