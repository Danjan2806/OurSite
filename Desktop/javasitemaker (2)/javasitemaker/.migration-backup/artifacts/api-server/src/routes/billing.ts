...
router.post("/billing/subscribe", async (req, res) => {
  ...
  try {
    ...
    if (plan === "free") {
      ...
      return res.json({ plan: user.plan, message: "Тариф изменён на Free" });
    }
    ...
    if (isTrial) {
      ...
      const [user] = await db.select().from(usersTable).where(eq(usersTable.id, userId)).limit(1);
      return res.json({ plan: user.plan, billing, message: `Пробный период тарифа ${plan} активирован` });
    }
    ...
    return res.json({
      plan: user.plan,
      billing,
      message: `Тариф ${plan} успешно активирован`,
    });
  } catch (err) {
