...
function RenderedElement({ el, bp, scrollY, containerRef, onOpenPopup }:
  { el: ZeroElement; bp: Breakpoint; scrollY: number; containerRef: React.RefObject<HTMLDivElement | null>; onOpenPopup?: (id: string) => void }) {
  ...
  return (
    <div ref={elRef} style={baseStyle}
      ...
      {el.type === "button" && (() => {
        const isPopup = typeof c.href === "string" && c.href.startsWith("#popup:");
        const popupId = isPopup ? (c.href as string).slice(7) : "";
        ...
        return isPopup ? (
          <button type="button" onClick={() => onOpenPopup?.(popupId)} style={{ ...btnStyle, border: "none" }}>
            {c.label || "Кнопка"}
          </button>
        ) : (
          <a href={c.href || "#"} target={c.target || "_self"} rel="noopener noreferrer" style={btnStyle}>
            {c.label || "Кнопка"}
          </a>
        );
      })()}
...
export default function ZeroBlockRenderer({ content, styles, onOpenPopup }: { content: any; styles?: any; onOpenPopup?: (id: string) => void }) {
  ...
  return (
    <div ref={containerRef} style={{ width: "100%", height: data.canvasHeight, position: "relative", overflow: "hidden", background: data.bg && data.bg !== "transparent" ? data.bg : undefined }}>
      {sorted.map(el => (
        <RenderedElement key={el.id} el={el} bp={bp} scrollY={scrollY} containerRef={containerRef} onOpenPopup={onOpenPopup} />
      ))}
    </div>
  );
}
