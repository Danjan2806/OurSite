...
import {
  ...
  MapPin, Code2, X, Star, Package, Bell
} from "@/lib/icons";
...
const DEFAULT_CART_STYLE: CartStyle = {
  icon: "bag", iconColor: "", badgeColor: "#ec4899",
  bg: "#111121", accent: "#7c3aed", btnText: "Оформить заказ →", showLabel: false,
};
const CartStyleContext = createContext<CartStyle>(DEFAULT_CART_STYLE);

// ─── Cart icon resolver ───────────────────────────────────────────────────────
function CartIconEl({ name, size, className }: { name?: string; size?: number; className?: string }) {
  if (name === "star") return <Star size={size} className={className} />;
  if (name === "box")  return <Package size={size} className={className} />;
  if (name === "bell") return <Bell size={size} className={className} />;
    ...
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm px-0 sm:px-4" onClick={e => { if (e.target === e.currentTarget && step !== "success") onClose(); }}>
      <div className="bg-[#111121] border border-white/10 rounded-t-3xl sm:rounded-2xl w-full max-w-md shadow-2xl relative flex flex-col max-h-[95vh]">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-white/8 flex-shrink-0">
          ...
          {step !== "success" && (
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/8 hover:bg-white/15 text-white/50 hover:text-white transition flex items-center justify-center"><X size={14} /></button>
          )}
        </div>
...
function CartDrawer({ siteId, currency, onClose }: { siteId: string; currency: string; onClose: () => void }) {
  ...
  const cs = useContext(CartStyleContext);
  ...
  const accent = cs.accent || "#7c3aed";
  const drawerBg = cs.bg || "#111121";

  return (
    <>
      <div className="fixed inset-0 z-[150] bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="fixed top-0 right-0 bottom-0 z-[160] w-full max-w-sm border-l border-white/10 flex flex-col shadow-2xl"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/8">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center"
              ...
              <CartIconEl name={cs.icon} size={17} style={{ color: accent }} />
            </div>
            <div>
          ...
          {cart.items.length === 0 ? (
            <div className="text-center py-16 space-y-3">
              <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mx-auto">
                <CartIconEl name={cs.icon} size={28} className="text-white/20" />
              </div>
            ...
            <div key={item.id} className="flex gap-3 items-start bg-white/4 rounded-xl p-3 border border-white/6">
              <div className="w-14 h-14 rounded-xl bg-white/5 flex-shrink-0 overflow-hidden flex items-center justify-center">
                {item.image
                  ...
                  : <CartIconEl name={cs.icon} size={18} className="text-white/15" />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-medium leading-tight truncate">{item.name}</p>
                <p className="text-sm font-bold mt-0.5" style={{ color: accent }}>{item.price}</p>
              ...
              <button onClick={() => cart.remove(item.id)} className="text-white/15 hover:text-red-400 transition flex-shrink-0 mt-0.5 w-6 h-6 flex items-center justify-center"><X size={13} /></button>
            </div>
          ))}
        </div>
...
function CartNavIcon() {
  ...
  return (
    <button onClick={cart.openCart}
      className="relative flex items-center gap-1.5 px-2 h-9 rounded-xl hover:bg-current/10 transition flex-shrink-0"
      style={cs.iconColor ? { color: cs.iconColor } : undefined}
      title={`Корзина${count > 0 ? ` (${count})` : ""}`}>
      <CartIconEl name={cs.icon} size={18} />
      {cs.showLabel && <span className="text-xs font-semibold">Корзина</span>}
      {count > 0 && (
        <span className="absolute -top-1 -right-1 w-4.5 h-4.5 rounded-full text-white text-[9px] font-bold flex items-center justify-center min-w-[18px] px-0.5"
...
/** Render a link that can either navigate or open a popup.
 *  Popup links use the format #popup:popupId */
function SmartLink({ href, children, className, style, onClick }: {
  href: string; children: React.ReactNode;
  className?: string; style?: React.CSSProperties; onClick?: () => void;
}) {
  const popup = usePopup();
  if (href?.startsWith("#popup:")) {
    const popupId = href.slice(7);
    return (
      <button type="button" className={className} style={style}
        onClick={() => { popup?.openPopup(popupId); onClick?.(); }}>
        {children}
      </button>
    );
  }
...
// ─── Rich text helper ─────────────────────────────────────────────────────────
function RH({ html, className, style }: { html?: string; className?: string; style?: React.CSSProperties }) {
  if (!html) return null;
  const isHTML = /<[a-zA-Z]/.test(html);
  if (isHTML) return <div className={`prose-like ${className || ""}`} style={style} dangerouslySetInnerHTML={{ __html: html }} />;
  return <p className={className} style={style}>{html}</p>;
}

// ─── Video URL → Embed URL converter ─────────────────────────────────────────
function toEmbed(url: string): string {
  if (!url) return url;
  // Already embed — pass through
  if (url.includes("/embed/") || url.includes("video_ext.php") || url.includes("player.vimeo") || url.includes("vk.com/video_ext")) return url;
  // YouTube Shorts
  ...
  // VK clips page: vk.com/clips-?z=clip-OID_VID
  const vkZ = url.match(/[?&]z=clip(-?\d+)_(\d+)/);
  if (vkZ) return `https://vkvideo.ru/video_ext.php?oid=${vkZ[1]}&id=${vkZ[2]}&hd=2`;
  // VK video/clip: vk.com/video-OID_VID, vk.com/clip-OID_VID, vkvideo.ru/video-OID_VID, vkvideo.ru/clip-OID_VID
  const vkV = url.match(/(?:vk\.com|vkvideo\.ru)\/(?:video|clip)(-?\d+)_(\d+)/);
  if (vkV) return `https://vkvideo.ru/video_ext.php?oid=${vkV[1]}&id=${vkV[2]}&hd=2`;
  return url;
}

// ─── Shorts Slider Block ──────────────────────────────────────────────────────
function ShortsSliderBlock({ anchorId, bgStyle, titleSt, s, c, videos }: {
  anchorId?: string; bgStyle: React.CSSProperties; titleSt: React.CSSProperties;
  s: any; c: any; videos: { url: string; title?: string }[];
}) {
  const [cur, setCur] = useState(0);
  const count = videos.length;
  const go = (dir: number) => setCur(prev => (prev + dir + count) % count);
  const isVideoFile = (url: string) => /\.(mp4|webm|mov|m4v|ogv)(\?|$)/i.test(url) || url.includes("/storage/objects/");
  const curUrl = count > 0 ? videos[cur]?.url || "" : "";
  const embedUrl = curUrl && !isVideoFile(curUrl) ? toEmbed(curUrl) : curUrl;
  return (
    <div id={anchorId} style={bgStyle} className="w-full py-16 px-8 flex flex-col items-center">
      {c.title && <h2 style={{ color: s.textColor || "#fff", ...titleSt }} className="text-3xl font-bold text-center mb-8">{c.title}</h2>}
      {count === 0
        ? <div className="bg-black/50 rounded-2xl overflow-hidden flex flex-col items-center justify-center text-white/20 gap-3" style={{ width: 320, aspectRatio: "9/16" }}>
            <Video size={48} /><p className="text-sm">Shorts / Reels / VK Clips 9:16</p>
          </div>
        : <div className="relative flex flex-col items-center" style={{ width: 320 }}>
            <div className="bg-black/50 rounded-2xl overflow-hidden w-full" style={{ aspectRatio: "9/16", position: "relative" }}>
              {!curUrl
                ? <div className="absolute inset-0 flex items-center justify-center text-white/20"><Video size={48} /></div>
                : isVideoFile(curUrl)
                ? <video key={curUrl} src={curUrl} className="absolute inset-0 w-full h-full object-cover" controls playsInline loop />
                : <iframe key={embedUrl} src={embedUrl} className="absolute inset-0 w-full h-full" allowFullScreen allow="autoplay; encrypted-media" />}
            </div>
            {count > 1 && <>
              <button onClick={() => go(-1)}
                className="absolute top-1/2 -translate-y-1/2 -left-10 w-9 h-9 rounded-full bg-black/50 flex items-center justify-center text-white/80 hover:bg-black/70 transition text-xl">‹</button>
              <button onClick={() => go(1)}
                className="absolute top-1/2 -translate-y-1/2 -right-10 w-9 h-9 rounded-full bg-black/50 flex items-center justify-center text-white/80 hover:bg-black/70 transition text-xl">›</button>
              <div className="flex justify-center gap-2 mt-4">
                {videos.map((_, i) => (
                  <button key={i} onClick={() => setCur(i)} className="rounded-full transition-all h-2"
                    style={{ width: i === cur ? 24 : 7, backgroundColor: i === cur ? (s.ctaColor || "#7c3aed") : "rgba(255,255,255,0.3)" }} />
                ))}
              </div>
            </>}
            {videos[cur]?.title && <p style={{ color: s.textColor || "#fff" }} className="text-sm opacity-60 text-center mt-3">{videos[cur].title}</p>}
          </div>
      }
      {c.description && <RH html={c.description} style={{ color: s.textColor || "#fff" }} className="text-base opacity-60 text-center mt-6 max-w-xl mx-auto" />}
    </div>
  );
}

// ─── Shorts Slider Standalone Block (SHORTS_SLIDER block type) ────────────────
function ShortsSliderStandaloneBlock({ anchorId, bgStyle, titleSt, s, c }: {
  anchorId?: string; bgStyle: React.CSSProperties; titleSt: React.CSSProperties;
  s: any; c: any;
}) {
  const [cur, setCur] = useState(0);
  ...
  const count = videos.length;
  const go = (dir: number) => setCur(prev => (prev + dir + Math.max(count, 1)) % Math.max(count, 1));
  const isVideoFile = (url: string) => /\.(mp4|webm|mov|m4v|ogv)(\?|$)/i.test(url) || url.includes("/storage/objects/");
  const variant = s.variant || "peek";
  const accentColor = s.ctaColor || "#7c3aed";
  const textColor = s.textColor || "#ffffff";

  const renderClip = (idx: number, isActive: boolean) => {
    const clip = videos[idx];
    if (!clip) return <div className="absolute inset-0 flex flex-col items-center justify-center text-white/20 gap-2"><Video size={isActive ? 36 : 20} /></div>;
    const url = clip.url || "";
    if (!url) return <div className="absolute inset-0 flex flex-col items-center justify-center text-white/20 gap-2"><Video size={isActive ? 36 : 20} /></div>;
    if (isVideoFile(url)) return <video key={url} src={url} className="absolute inset-0 w-full h-full object-cover" controls={isActive} playsInline loop />;
    return <iframe key={url} src={toEmbed(url)} className="absolute inset-0 w-full h-full" allowFullScreen allow="autoplay; encrypted-media" />;
  };

  if (variant === "full") {
    return (
      <div id={anchorId} style={bgStyle} className="w-full py-16 px-6 flex flex-col items-center">
        {c.title && <h2 style={{ color: textColor, ...titleSt }} className="text-3xl font-bold text-center mb-4">{c.title}</h2>}
        {c.subtitle && <RH html={c.subtitle} style={{ color: textColor }} className="text-base opacity-70 text-center mb-8 max-w-xl mx-auto" />}
        <div className="relative flex flex-col items-center" style={{ width: 340 }}>
          <div className="bg-black/60 rounded-3xl overflow-hidden w-full shadow-2xl" style={{ aspectRatio: "9/16", position: "relative" }}>
            {count === 0
              ? <div className="absolute inset-0 flex flex-col items-center justify-center text-white/20 gap-3"><Video size={48} /><p className="text-sm">Shorts / Reels / VK Clips 9:16</p></div>
              : renderClip(cur, true)}
          </div>
          {count > 1 && <>
            <button onClick={() => go(-1)} className="absolute top-1/2 -translate-y-1/2 -left-12 w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white/90 hover:bg-black/70 text-2xl transition">‹</button>
            <button onClick={() => go(1)} className="absolute top-1/2 -translate-y-1/2 -right-12 w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white/90 hover:bg-black/70 text-2xl transition">›</button>
            <div className="flex justify-center gap-2 mt-5">
              {videos.map((_, i) => <button key={i} onClick={() => setCur(i)} className="rounded-full transition-all h-2" style={{ width: i === cur ? 28 : 7, backgroundColor: i === cur ? accentColor : "rgba(255,255,255,0.3)" }} />)}
            </div>
          </>}
          {videos[cur]?.title && <p style={{ color: textColor }} className="text-sm opacity-60 text-center mt-3">{videos[cur].title}</p>}
        </div>
      </div>
    );
  }

  // peek & stack variants — show center + two side clips
  const prevIdx = count > 0 ? (cur - 1 + count) % count : -1;
  const nextIdx = count > 0 ? (cur + 1) % count : -1;
  const sideW = variant === "stack" ? 110 : 120;
  const centerW = variant === "stack" ? 240 : 260;

  return (
    <div id={anchorId} style={bgStyle} className="w-full py-16 overflow-hidden flex flex-col items-center">
      {c.title && <h2 style={{ color: textColor, ...titleSt }} className="text-3xl font-bold text-center mb-4 px-6">{c.title}</h2>}
      {c.subtitle && <RH html={c.subtitle} style={{ color: textColor }} className="text-base opacity-70 text-center mb-10 max-w-xl mx-auto px-6" />}

      {count === 0
        ? <div className="bg-black/50 rounded-3xl overflow-hidden flex flex-col items-center justify-center text-white/20 gap-3" style={{ width: centerW, aspectRatio: "9/16" }}>
            <Video size={48} /><p className="text-sm text-center">Shorts / Reels / VK Clips 9:16</p>
          </div>
        : <div className="relative flex items-center justify-center w-full" style={{ height: centerW * 16/9 + 24 }}>
            {/* Prev card */}
            {count > 1 && (
              <div onClick={() => go(-1)} className="absolute cursor-pointer" style={{ right: `calc(50% + ${centerW/2 + 8}px)`, top: "50%", transform: "translateY(-50%)", width: sideW, aspectRatio: "9/16", zIndex: 1 }}>
                <div className="w-full h-full bg-black/60 rounded-2xl overflow-hidden opacity-50 hover:opacity-70 transition-opacity shadow-lg" style={{ position: "relative" }}>
                  {renderClip(prevIdx, false)}
                  <div className="absolute inset-0 bg-black/30" />
                </div>
              </div>
            )}
            {/* Center card */}
            <div className="relative z-10 shadow-2xl rounded-3xl overflow-hidden" style={{ width: centerW, aspectRatio: "9/16" }}>
              {renderClip(cur, true)}
              ...
              {videos[cur]?.title && (
                <div className="absolute bottom-4 left-0 right-0 text-center px-4">
                  <p className="text-white text-sm font-medium drop-shadow">{videos[cur].title}</p>
                </div>
              )}
            </div>
            {/* Next card */}
            {count > 1 && (
              <div onClick={() => go(1)} className="absolute cursor-pointer" style={{ left: `calc(50% + ${centerW/2 + 8}px)`, top: "50%", transform: "translateY(-50%)", width: sideW, aspectRatio: "9/16", zIndex: 1 }}>
                <div className="w-full h-full bg-black/60 rounded-2xl overflow-hidden opacity-50 hover:opacity-70 transition-opacity shadow-lg" style={{ position: "relative" }}>
                  {renderClip(nextIdx, false)}
                  <div className="absolute inset-0 bg-black/30" />
                </div>
              </div>
            )}
            {/* Nav arrows */}
            {count > 1 && <>
              <button onClick={() => go(-1)} className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white/90 hover:bg-black/70 text-2xl transition shadow-lg">‹</button>
              <button onClick={() => go(1)} className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white/90 hover:bg-black/70 text-2xl transition shadow-lg">›</button>
            </>}
          </div>
      }
      {count > 1 && (
...
// ─── Video Slider Block ───────────────────────────────────────────────────────
function VideoSliderBlock({ anchorId, bgStyle, titleSt, s, c, videos }: {
  anchorId?: string; bgStyle: React.CSSProperties; titleSt: React.CSSProperties;
  s: any; c: any; videos: { url: string; title?: string }[];
}) {
  const [cur, setCur] = useState(0);
  ...
  return (
    <div id={anchorId} style={bgStyle} className="w-full py-16 px-8">
      {c.title && <h2 style={{ color: s.textColor || "#fff", ...titleSt }} className="text-3xl font-bold text-center mb-8">{c.title}</h2>}
      {count === 0
        ? <div className="max-w-3xl mx-auto bg-black/40 rounded-2xl flex items-center justify-center" style={{ aspectRatio: "16/9" }}>
            <Video size={48} className="text-white/20" />
          </div>
        : <div className="max-w-3xl mx-auto">
            <div className="relative">
              <div className="bg-black/50 rounded-2xl overflow-hidden" style={{ aspectRatio: "16/9", position: "relative" }}>
                {videos[cur]?.url
                  ? <iframe key={toEmbed(videos[cur].url)} src={toEmbed(videos[cur].url)} className="absolute inset-0 w-full h-full" allowFullScreen allow="autoplay; encrypted-media" />
                  : <div className="absolute inset-0 flex items-center justify-center text-white/20"><Video size={48} /></div>}
              </div>
              {count > 1 && <>
                <button onClick={() => go(-1)} className="absolute inset-y-0 left-0 w-14 flex items-center justify-center bg-black/40 text-white/80 rounded-l-2xl hover:bg-black/60 transition">
                  <span style={{ fontSize: 32 }}>‹</span>
                </button>
                <button onClick={() => go(1)} className="absolute inset-y-0 right-0 w-14 flex items-center justify-center bg-black/40 text-white/80 rounded-r-2xl hover:bg-black/60 transition">
                  <span style={{ fontSize: 32 }}>›</span>
                </button>
              </>}
            </div>
            {videos[cur]?.title && <p style={{ color: s.textColor || "#fff" }} className="text-sm opacity-60 text-center mt-3">{videos[cur].title}</p>}
            {count > 1 && <div className="flex justify-center gap-2 mt-4">
              {videos.map((_, i) => (
                <button key={i} onClick={() => setCur(i)} className="rounded-full transition-all h-2"
                  style={{ width: i === cur ? 28 : 8, backgroundColor: i === cur ? (s.ctaColor || "#7c3aed") : "rgba(255,255,255,0.3)" }} />
              ))}
            </div>}
          </div>
      }
      {c.description && <p style={{ color: s.textColor || "#fff" }} className="text-base opacity-60 text-center mt-6 max-w-xl mx-auto">{c.description}</p>}
    </div>
  );
}

// ─── Gallery Slider ───────────────────────────────────────────────────────────
function GallerySlider({ images, s }: { images: { url: string; caption?: string }[]; s: any }) {
  const [cur, setCur] = useState(0);
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const [hovered, setHovered] = useState(false);
  const speed = s.sliderSpeed || "0.4s";
  const autoplay = parseInt(s.sliderAutoplay || "0");
  const dots = s.sliderDots || "dots";
  const dotColor = s.sliderDotColor || "#7c3aed";
  const dotOpacity = s.sliderDotOpacity ?? 0.35;
  const arrows = s.sliderArrows || "always";
  const arrowColor = s.sliderArrowColor || "#ffffff";
  const arrowOpacity = s.sliderArrowOpacity ?? 0.7;
  const arrowHoverOpacity = s.sliderArrowHoverOpacity ?? 1;
  const count = images.length;

  useEffect(() => {
    if (autoplay > 0 && count > 1) {
      timerRef.current = setInterval(() => setCur(c => (c + 1) % count), autoplay);
    }
  ...
  const go = (dir: number) => {
    ...
    setCur(c => (c + dir + count) % count);
  };

  const showArrows = arrows === "always" || (arrows === "hover" && hovered);

  return (
    <div className="max-w-5xl mx-auto" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className="relative rounded-2xl overflow-hidden" style={{ aspectRatio: "16/9" }}>
        <div className="flex h-full transition-transform" style={{ transform: `translateX(-${cur * 100}%)`, transitionDuration: speed }}>
          {images.map((img, i) => (
            <img key={i} src={img.url} className="w-full h-full object-cover flex-shrink-0" style={{ minWidth: "100%" }} />
          ))}
        </div>
        {showArrows && count > 1 && <>
          <button onClick={() => go(-1)}
            className="absolute inset-y-0 left-0 w-14 flex items-center justify-center transition-opacity"
            style={{ backgroundColor: "rgba(0,0,0,0.35)", color: arrowColor, opacity: hovered ? arrowHoverOpacity : arrowOpacity }}>
            <span style={{ fontSize: 32, lineHeight: 1 }}>‹</span>
          </button>
          <button onClick={() => go(1)}
            className="absolute inset-y-0 right-0 w-14 flex items-center justify-center transition-opacity"
            style={{ backgroundColor: "rgba(0,0,0,0.35)", color: arrowColor, opacity: hovered ? arrowHoverOpacity : arrowOpacity }}>
            <span style={{ fontSize: 32, lineHeight: 1 }}>›</span>
          </button>
        </>}
      ...
      {dots !== "none" && count > 1 && (
        <div className="flex justify-center items-center gap-2 mt-4">
          {dots === "nums"
            ? <span style={{ color: dotColor }} className="text-sm font-semibold">{cur + 1} / {count}</span>
            : images.map((_, i) => (
                <button key={i} onClick={() => { go(0); setCur(i); }}
                  className={`rounded-full transition-all ${dots === "bars" ? "h-1.5" : "h-2"}`}
                  style={{
                    width: dots === "bars" ? (i === cur ? 28 : 12) : (i === cur ? 10 : 7),
                    backgroundColor: dotColor,
                    opacity: i === cur ? 1 : dotOpacity,
                  }} />
              ))
          }
        </div>
...
// ─── Schedule Block ────────────────────────────────────────────────────────────
function ScheduleBlock({ anchorId, bgStyle, titleSt, s, c }: {
  anchorId?: string; bgStyle: React.CSSProperties; titleSt: React.CSSProperties;
  s: any; c: any;
}) {
  const schv = s.variant || "list";
  const accentClr = s.ctaColor || "#7c3aed";
  const textColor = s.textColor || "#e2e8f0";
  const items: any[] = c.items || [];

  const showTrainer    = s.showTrainer    !== false;
  const showDuration   = s.showDuration   !== false;
  const showCapacity   = !!s.showCapacity;
  const showLevel      = !!s.showLevel;
  const showLocation   = s.showLocation   !== false;
  const showModal      = s.showModal      !== false;
  const showNavigation = s.showNavigation !== false;
  const ctaLabel       = c.ctaLabel || "Записаться";
  const detailsLabel   = c.detailsLabel || "Подробнее";

  // Calendar state
  const parseStartMonth = () => {
    if (c.startMonth) { const [y,m] = c.startMonth.split("-").map(Number); if (y && m) return new Date(y, m-1, 1); }
    return new Date();
  };
  const [calDate, setCalDate] = useState(parseStartMonth);
  const [modal, setModal] = useState<{ events: any[]; day: number } | null>(null);
  const [expandedIdx, setExpandedIdx] = useState<number | null>(null);

  const DAYS_RU = ["Пн","Вт","Ср","Чт","Пт","Сб","Вс"];
  const monthNames = ["Январь","Февраль","Март","Апрель","Май","Июнь","Июль","Август","Сентябрь","Октябрь","Ноябрь","Декабрь"];
  const DOW_MAP: Record<number, string[]> = {
    0:["пн","пон","mon"],1:["вт","вторник","tue"],2:["ср","среда","wed"],
    3:["чт","четверг","thu"],4:["пт","пятница","fri"],5:["сб","суббота","sat"],6:["вс","воскресенье","sun"]
  };

  const itemColor = (it: any) => it.color || accentClr;

  const itemMatchesDow = (it: any, dowIdx: number) => {
    const dStr = (it.day || "").toLowerCase();
    return (DOW_MAP[dowIdx] || []).some(m => dStr.includes(m));
  };

  const itemMatchesDate = (it: any, dayNum: number) => {
    const dStr = (it.day || "").trim();
    if (!dStr) return false;
    const nums = dStr.split(/[,\s]+/).map((x: string) => parseInt(x.trim())).filter((n: number) => !isNaN(n));
    if (nums.length > 0) return nums.includes(dayNum);
    return false;
  };

  const getEventsForDate = (dayNum: number): any[] => {
    return items.filter(it => itemMatchesDate(it, dayNum));
  };

  const getEventsForDow = (dowIdx: number): any[] => {
    return items.filter(it => itemMatchesDow(it, dowIdx));
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
        ...
        <div className="relative z-10 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl"
          ...
          <div className="px-6 pt-5 pb-4 flex items-center justify-between border-b border-white/8">
            <span className="text-white font-bold text-lg">{monthNames[calDate.getMonth()].replace("ь","я").replace("рь","ря") || ""} {modal.day}</span>
            <button onClick={() => setModal(null)} className="w-8 h-8 rounded-full bg-white/8 flex items-center justify-center text-white/60 hover:bg-white/15 transition text-lg">×</button>
          </div>
          <div className="px-6 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
            {modal.events.map((it, i) => (
              <div key={i} className="rounded-2xl p-4" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderLeft: `4px solid ${itemColor(it)}` }}>
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    <p className="text-white font-bold text-base">{it.type}</p>
                    {it.time && <p className="text-sm mt-0.5" style={{ color: itemColor(it) }}>{it.time}{showDuration && it.duration ? ` · ${it.duration}` : ""}</p>}
                  </div>
                  {it.ctaUrl && <a href={it.ctaUrl} target="_blank" rel="noopener noreferrer"
                    className="flex-shrink-0 text-xs px-3 py-1.5 rounded-full font-semibold whitespace-nowrap"
                    style={{ backgroundColor: itemColor(it)+"25", color: itemColor(it) }}>{it.ctaLabel || ctaLabel}</a>}
                </div>
                <div className="space-y-1 text-sm text-white/60">
                  {showTrainer && it.trainer && <p>👤 {it.trainer}</p>}
                  {showLocation && it.location && <p>📍 {it.location}</p>}
                  {showCapacity && it.capacity && <p>👥 {it.capacity}</p>}
                  {showLevel && it.level && <p>⭐ {it.level}</p>}
                </div>
                {it.description && (
                  <div className="mt-3 pt-3 border-t border-white/8">
                    <RH html={it.description} className="text-sm text-white/70 prose-like" />
                  </div>
                )}
              </div>
            ))}
  ...
  if (schv === "month") {
    ...
    const goMonth = (dir: number) => setCalDate(d => new Date(d.getFullYear(), d.getMonth() + dir, 1));

    return (
      <div id={anchorId} style={{ ...bgStyle, color: textColor }} className="w-full py-16 px-8">
        {modal && <EventModal />}
        <div className="max-w-5xl mx-auto">
          {c.title && <h2 style={{ ...titleSt }} className="text-3xl font-bold mb-2">{c.title}</h2>}
          {c.subtitle && <RH html={c.subtitle} className="text-base opacity-60 mb-8 max-w-2xl" style={{ color: textColor }} />}
          <div className="flex items-center justify-between mb-6">
            <span className="text-xl font-bold" style={{ color: textColor }}>{monthNames[month]} {year}</span>
            {showNavigation && (
              <div className="flex gap-2">
                <button onClick={() => goMonth(-1)} className="w-9 h-9 rounded-full flex items-center justify-center text-lg transition hover:bg-white/10" style={{ color: textColor, border: "1px solid rgba(255,255,255,0.15)" }}>‹</button>
                <button onClick={() => goMonth(1)} className="w-9 h-9 rounded-full flex items-center justify-center text-lg transition hover:bg-white/10" style={{ color: textColor, border: "1px solid rgba(255,255,255,0.15)" }}>›</button>
              </div>
            )}
          </div>
          <div className="grid grid-cols-7 gap-1.5">
            {DAYS_RU.map(d => <div key={d} className="text-center text-xs font-bold py-2 opacity-40" style={{ color: textColor }}>{d}</div>)}
            {cells.map((day, i) => {
              ...
              return (
                <div key={i}
                  onClick={() => hasEvents && showModal && setModal({ events, day: day! })}
                  className={`rounded-xl p-2 min-h-[90px] transition-all ${!day ? "opacity-0 pointer-events-none" : ""} ${hasEvents && showModal ? "cursor-pointer hover:bg-white/8" : ""}`}
                  style={{ backgroundColor: day ? (isToday ? accentClr+"18" : "rgba(255,255,255,0.03)") : "transparent", border: `1px solid ${hasEvents ? accentClr+"35" : "transparent"}` }}>
                  {day && <>
                    <div className={`text-sm font-bold mb-1 w-7 h-7 flex items-center justify-center rounded-full`}
                      style={isToday ? { backgroundColor: accentClr, color: "#fff" } : hasEvents ? { color: accentClr } : { opacity: 0.5, color: textColor }}>
                      {day}
                    </div>
                    {events.slice(0, 2).map((it, ii) => (
                      <div key={ii} className="text-[10px] rounded-md px-1.5 py-0.5 truncate mb-0.5 font-medium"
                        style={{ backgroundColor: itemColor(it)+"22", color: itemColor(it) }}>
                        {it.time && <span className="opacity-80">{it.time} </span>}{it.type}
                      </div>
                    ))}
                    {events.length > 2 && (
                      <div className="text-[10px] font-medium opacity-50" style={{ color: accentClr }}>+{events.length-2} ещё</div>
                    )}
                  </>}
                </div>
              );
          ...
          {items.length > 0 && (
            <div className="mt-8 pt-6 border-t border-white/8">
              <p className="text-sm font-semibold opacity-50 mb-3">Все занятия</p>
              <div className="flex flex-wrap gap-3">
                {items.map((it, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm rounded-full px-3 py-1"
                    style={{ backgroundColor: itemColor(it)+"18", color: itemColor(it) }}>
                    <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: itemColor(it) }} />
      ...
      <div id={anchorId} style={{ ...bgStyle, color: textColor }} className="w-full py-16 px-8">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="overflow-x-auto">
            <div className="grid min-w-[700px]" style={{ gridTemplateColumns: "repeat(7,1fr)", gap: "6px" }}>
              {DAYS_RU.map((d, i) => (
                <div key={i} className="text-center text-sm font-bold py-2.5 rounded-xl" style={{ backgroundColor: "rgba(255,255,255,0.06)", color: textColor }}>{d}</div>
              ))}
              {DAYS_RU.map((_, di) => {
                const dayItems = getEventsForDow(di);
                return (
                  <div key={di} className="rounded-xl p-2 min-h-[140px] space-y-2" style={{ backgroundColor: "rgba(255,255,255,0.025)" }}>
                    {dayItems.map((it, ii) => (
                      <div key={ii}
                        onClick={() => showModal && setModal({ events: [it], day: 0 })}
                        className={`text-xs rounded-xl px-2.5 py-2 transition ${showModal ? "cursor-pointer hover:opacity-85" : ""}`}
                        style={{ backgroundColor: itemColor(it)+"22", borderLeft: `3px solid ${itemColor(it)}` }}>
                        <p className="font-bold" style={{ color: itemColor(it) }}>{it.time}</p>
                        <p className="font-semibold mt-0.5" style={{ color: textColor }}>{it.type}</p>
                        {showTrainer && it.trainer && <p className="opacity-50 mt-0.5" style={{ color: textColor }}>{it.trainer}</p>}
                        {showDuration && it.duration && <p className="opacity-40 mt-0.5" style={{ color: textColor }}>{it.duration}</p>}
                        {showLocation && it.location && <p className="opacity-40 mt-0.5" style={{ color: textColor }}>📍 {it.location}</p>}
                        {showModal && it.description && (
                          <p className="mt-1.5 text-[10px] font-medium" style={{ color: itemColor(it) }}>{detailsLabel} →</p>
                        )}
                      </div>
                    ))}
                  </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-2">
          {items.map((it, i) => {
            ...
            const hasDetails = !!(it.description || it.duration || it.capacity || it.level || it.location);
            return (
              <div key={i} className="rounded-2xl overflow-hidden transition-all"
                style={{ backgroundColor: "rgba(255,255,255,0.04)", border: `1px solid ${isExpanded ? itemColor(it)+"40" : "rgba(255,255,255,0.06)"}` }}>
                <div className={`flex flex-wrap items-center gap-4 px-5 py-4 ${hasDetails ? "cursor-pointer" : ""}`}
                  onClick={() => hasDetails && setExpandedIdx(isExpanded ? null : i)}>
                  <div className="w-1 h-10 rounded-full flex-shrink-0" style={{ backgroundColor: itemColor(it) }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-base font-bold" style={{ color: textColor }}>{it.type}</p>
                    <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
                      {showTrainer && it.trainer && <p className="text-sm opacity-50">{it.trainer}</p>}
                      {showLocation && it.location && <p className="text-sm opacity-40">📍 {it.location}</p>}
                    </div>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <div className="text-right">
                      <p className="text-sm font-bold" style={{ color: itemColor(it) }}>{it.time}</p>
                      {showDuration && it.duration && <p className="text-xs opacity-40">{it.duration}</p>}
                    </div>
                    <p className="text-sm opacity-40">{it.day}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {showCapacity && it.capacity && <span className="text-xs rounded-full px-2 py-0.5" style={{ backgroundColor: accentClr+"18", color: accentClr }}>{it.capacity}</span>}
                    {showLevel && it.level && <span className="text-xs rounded-full px-2 py-0.5 bg-white/6 opacity-60" style={{ color: textColor }}>{it.level}</span>}
                    {it.ctaUrl && <a href={it.ctaUrl} target="_blank" rel="noopener noreferrer"
                      onClick={e => e.stopPropagation()}
                      className="text-sm px-4 py-1.5 rounded-full font-semibold whitespace-nowrap"
                      style={{ backgroundColor: itemColor(it)+"22", color: itemColor(it) }}>{it.ctaLabel || ctaLabel}</a>}
                    {hasDetails && <span className="opacity-30 text-lg transition-transform" style={{ transform: isExpanded ? "rotate(90deg)" : "none" }}>›</span>}
                  </div>
                </div>
                {isExpanded && (
                  <div className="px-5 pb-5 border-t border-white/6">
                    <div className="pt-4 flex flex-wrap gap-4 mb-3">
                      {showDuration && it.duration && <div className="text-sm"><span className="opacity-40">Длительность </span><span className="font-semibold">{it.duration}</span></div>}
                      {showCapacity && it.capacity && <div className="text-sm"><span className="opacity-40">Мест </span><span className="font-semibold">{it.capacity}</span></div>}
                      {showLevel && it.level && <div className="text-sm"><span className="opacity-40">Уровень </span><span className="font-semibold">{it.level}</span></div>}
                      {showLocation && it.location && <div className="text-sm"><span className="opacity-40">Зал </span><span className="font-semibold">{it.location}</span></div>}
                    </div>
                    {it.description && <RH html={it.description} className="text-sm opacity-70 prose-like" style={{ color: textColor }} />}
                  </div>
                )}
              </div>
            );
...
function ProductsBlock({ block }: { block: Block }) {
  ...
  if (prdv === "list") return (
    <div id={s.anchorId} style={bgStyle} className="w-full py-16 px-8">
      {c.title && <h2 style={{ color: s.textColor || "#fff" }} className="text-3xl font-bold mb-3">{c.title}</h2>}
      {c.subtitle && <p style={{ color: s.textColor || "#fff" }} className="text-base opacity-50 mb-10">{c.subtitle}</p>}
      {items.length === 0
        ? <p className="text-white/20 text-center py-8">Добавьте товары в настройках блока</p>
        : <div className="max-w-4xl mx-auto space-y-3">
            {items.map((p: any, i: number) => (
              <div key={i} className="flex items-center gap-5 bg-white/5 rounded-2xl overflow-hidden border border-white/5 hover:border-purple-500/30 transition">
                <div className="w-28 h-24 flex-shrink-0 bg-white/4 overflow-hidden">
                  {p.image ? <img src={p.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><ShoppingBag size={24} className="text-white/15" /></div>}
                </div>
                <div className="flex-1 py-3 min-w-0">
                  <p style={{ color: s.textColor || "#fff" }} className="text-base font-semibold">{p.name}</p>
                  {p.description && <p className="text-sm opacity-40 mt-0.5 truncate" style={{ color: s.textColor || "#fff" }}>{p.description}</p>}
                </div>
                <div className="pr-5 flex items-center gap-4 flex-shrink-0">
                  <span className="font-bold text-lg" style={{ color: accentClr }}>{fmtPrice(p.price)}</span>
                  <button onClick={() => handleAdd(p, i)}
                    className={`text-sm py-2 px-5 rounded-xl font-semibold transition ${added[i] ? "bg-green-600 text-white" : "bg-purple-600 hover:bg-purple-500 text-white"}`}>
                    {added[i] ? "✓" : "В корзину"}
                  </button>
                </div>
              </div>
      ...
      <div id={s.anchorId} style={bgStyle} className="w-full py-16 px-8">
        ...
        {items.length === 0
          ...
          : <div className="max-w-5xl mx-auto flex gap-6">
              {feat && (
                <div className="flex-1 bg-white/6 rounded-2xl overflow-hidden border border-purple-500/30 flex flex-col">
                  <div className="h-72 bg-white/4 relative overflow-hidden">
                    {feat.image ? <img src={feat.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><ShoppingBag size={40} className="text-white/15" /></div>}
                    {feat.badge && <span className="absolute top-3 left-3 text-xs bg-purple-600 text-white px-2.5 py-0.5 rounded-full font-semibold">{feat.badge}</span>}
                  </div>
                  <div className="p-6 flex flex-col flex-1">
                    <p style={{ color: s.textColor || "#fff" }} className="text-xl font-bold flex-1">{feat.name}</p>
                    {feat.description && <p className="text-sm opacity-40 mt-2 mb-3" style={{ color: s.textColor || "#fff" }}>{feat.description}</p>}
                    <p className="font-bold text-2xl mt-2" style={{ color: accentClr }}>{fmtPrice(feat.price)}</p>
                    <button onClick={() => handleAdd(feat, 0)}
                      className={`mt-4 w-full text-sm py-3 rounded-xl text-center font-semibold transition ${added[0] ? "bg-green-600 text-white" : "bg-purple-600 hover:bg-purple-500 text-white"}`}>
                      {added[0] ? "✓ Добавлено" : "В корзину"}
                    </button>
                  </div>
                </div>
              )}
              <div className="grid grid-cols-2 gap-4 w-80">
                {rest.slice(0, 4).map((p: any, i: number) => (
                  <div key={i} className="bg-white/6 rounded-xl overflow-hidden border border-white/5 flex flex-col">
                    <div className="h-32 bg-white/4 overflow-hidden">
                      {p.image ? <img src={p.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><ShoppingBag size={20} className="text-white/15" /></div>}
                    </div>
                    <div className="p-3 flex flex-col flex-1">
                      <p style={{ color: s.textColor || "#fff" }} className="text-sm font-semibold flex-1">{p.name}</p>
                      <p className="font-bold text-base mt-1" style={{ color: accentClr }}>{fmtPrice(p.price)}</p>
                      <button onClick={() => handleAdd(p, i + 1)}
                        className={`mt-2 w-full text-xs py-2 rounded-lg font-semibold transition ${added[i+1] ? "bg-green-600 text-white" : "bg-purple-600 hover:bg-purple-500 text-white"}`}>
                        {added[i+1] ? "✓" : "В корзину"}
                      </button>
                    </div>
                  </div>
            ...
            {items.map((p: any, i: number) => (
              <div key={i} className="flex items-center justify-between py-5">
                <div className="flex-1 min-w-0 pr-6">
                  <p style={{ color: s.textColor || "#fff" }} className="text-base font-semibold">{p.name}</p>
                  {p.description && <p className="text-sm opacity-40 mt-1" style={{ color: s.textColor || "#fff" }}>{p.description}</p>}
                </div>
                <div className="flex items-center gap-4 flex-shrink-0">
                  <span className="font-bold text-xl" style={{ color: accentClr }}>{fmtPrice(p.price)}</span>
                  <button onClick={() => handleAdd(p, i)}
                    className={`text-sm w-10 h-10 rounded-xl font-bold transition flex items-center justify-center ${added[i] ? "bg-green-600 text-white" : "bg-purple-600/20 hover:bg-purple-600 text-purple-400 hover:text-white"}`}>
                    {added[i] ? "✓" : "+"}
                  </button>
                </div>
              </div>
...
function PopupRenderer({ popupBlocks, siteId }: { popupBlocks: Block[]; siteId: string }) {
  ...
  const imgPos = c.imagePosition || "top";
  const isImageLeft  = variant === "image-left" || imgPos === "left";
  const isImageRight = imgPos === "right";
  ...
  const innerStyle: React.CSSProperties = {
    ...
    display: "flex",
    flexDirection: isImageLeft ? "row" : isImageRight ? "row-reverse" : "column",
  };

  const contentBox = (
    <div className={`flex flex-col p-7 ${(isImageLeft || isImageRight) ? "flex-1 overflow-y-auto" : "overflow-y-auto"}`}>
        ...
        <div className={animClass} style={innerStyle}>
          {showClose && (
            <button onClick={popup.closePopup}
              ...
              style={{ flexShrink: 0 }}><X size={14} /></button>
          )}
          {c.image && imgPos === "background" && (
            <div className="absolute inset-0 z-0">{imageEl}<div className="absolute inset-0 bg-black/60" /></div>
          )}
          {c.image && (isImageLeft || isImageRight || imgPos === "top") && (
            <div className={`${(isImageLeft || isImageRight) ? "w-64 flex-shrink-0" : "h-48 w-full"} overflow-hidden`}>{imageEl}</div>
          )}
...
function eaItem(s: any, key: string, idx: number): { animation?: string; delay?: number; duration?: number } {
  const e = (s.elemAnims || {})[key] || {};
  if (!e.animation) return {};
  return { animation: e.animation, delay: (e.delay || 0) + idx * 0.1, duration: e.duration };
}

function elSt(s: any, key: string): React.CSSProperties {
  const e = (s.elemStyles || {})[key] || {};
  const out: React.CSSProperties = {};
  if (e.color) out.color = e.color;
  if (e.fontSize) out.fontSize = e.fontSize;
  if (e.fontWeight) out.fontWeight = e.fontWeight;
  if (e.align) out.textAlign = e.align as any;
  if (e.lineHeight) out.lineHeight = e.lineHeight;
  if (e.letterSpacing !== undefined && e.letterSpacing !== 0) out.letterSpacing = `${e.letterSpacing}px`;
  return out;
}
function elHover(s: any, key: string): string | undefined {
  return ((s.elemStyles || {})[key] || {}).hoverColor || undefined;
}
...
function LiveBlock({ block }: { block: Block }) {
  ...
  // Per-element style overrides (applied as inline styles on each key element)
  const titleSt = elSt(s, "title");
  const subSt   = elSt(s, "subtitle");
  const itSt    = elSt(s, "items");
  const ctaElSt = elSt(s, "cta");
  ...
  switch (block.type) {
    case "HERO": {
      ...
      if (hv === "split") return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff", minHeight: s.minHeight || "80vh" }} className="w-full flex">
          <div className="flex-1 flex flex-col justify-center px-12 py-20">
            {badgeEl}
            <AnimWrap {...ea(s,"title")}><h1 style={{ ...titleStyle, ...titleSt }} className="mb-4">{c.title || "Заголовок"}</h1></AnimWrap>
            <AnimWrap {...ea(s,"subtitle")}><p className="text-lg opacity-60 mb-8 max-w-lg" style={subSt}>{c.subtitle || "Подзаголовок"}</p></AnimWrap>
            {btns}
          </div>
          <AnimWrap {...ea(s,"image")}>
      ...
      if (hv === "minimal") return (
        <div id={anchorId} style={{ ...heroBg, color: s.textColor || "#111", minHeight: s.minHeight || "60vh" }} className="w-full py-24 px-8 flex flex-col items-center justify-center text-center">
          {badgeEl}
          <AnimWrap {...ea(s,"title")}><h1 style={{ ...titleStyle, fontWeight: "700", ...titleSt }} className="mb-4 max-w-2xl">{c.title || "Заголовок"}</h1></AnimWrap>
          <AnimWrap {...ea(s,"subtitle")}><p className="text-lg mb-8 max-w-xl" style={{ opacity: 0.5, ...subSt }}>{c.subtitle || "Подзаголовок"}</p></AnimWrap>
          {btns}
        </div>
      );
      return (
        <div id={anchorId} style={{ ...heroBg, color: s.textColor || "#fff", minHeight: s.minHeight || "80vh" }} className="w-full py-20 px-8 flex flex-col items-center justify-center text-center relative">
          ...
          <div className="relative z-10 flex flex-col items-center">
            {badgeEl}
            <AnimWrap {...ea(s,"title")}><h1 style={{ ...titleStyle, ...titleSt }} className="mb-4 max-w-3xl">{c.title || "Заголовок"}</h1></AnimWrap>
            <AnimWrap {...ea(s,"subtitle")}><p className="text-lg opacity-60 mb-8 max-w-xl" style={subSt}>{c.subtitle || "Подзаголовок"}</p></AnimWrap>
            {btns}
          </div>
        </div>
      );
    ...
    case "FEATURES": {
      ...
      if (fv === "list") return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#e2e8f0" }} className="w-full py-16 px-8">
          <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-3xl font-bold text-center mb-3">{c.title}</h2></AnimWrap>
          <AnimWrap {...ea(s,"subtitle")}><p style={subSt} className="text-base opacity-50 text-center mb-10 max-w-lg mx-auto">{c.subtitle}</p></AnimWrap>
          <div className="max-w-3xl mx-auto divide-y divide-white/8">
            {(c.items || []).map((it: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <div className="flex items-start gap-5 py-5">
                  ...
                  <div style={itSt}>
                    ...
                    <RH html={it.desc} className="text-sm opacity-50" style={itSt} />
                  </div>
                </div>
              </AnimWrap>
            ))}
      ...
      if (fv === "alternating") return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#e2e8f0" }} className="w-full py-16 px-8">
          <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-3xl font-bold text-center mb-3">{c.title}</h2></AnimWrap>
          <AnimWrap {...ea(s,"subtitle")}><p style={subSt} className="text-base opacity-50 text-center mb-12 max-w-lg mx-auto">{c.subtitle}</p></AnimWrap>
            ...
            {(c.items || []).map((it: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <div className={`flex items-center gap-12 ${i % 2 === 1 ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className="flex-1" style={itSt}>
                    <h3 className="text-2xl font-bold mb-3">{it.title}</h3>
                    <RH html={it.desc} className="text-base opacity-50" style={itSt} />
                  </div>
                </div>
              </AnimWrap>
            ))}
      ...
      return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#e2e8f0" }} className="w-full py-16 px-8">
          ...
          <div className="grid gap-6 max-w-5xl mx-auto" style={{ gridTemplateColumns: `repeat(${Math.min((c.items || []).length || 1, s.columns || 3)}, 1fr)` }}>
            {(c.items || []).map((it: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <div className="bg-white/5 rounded-2xl p-6 text-center h-full" style={itSt}>
      ...
      if (pv === "table") {
        ...
        return (
          <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff" }} className="w-full py-16 px-8 overflow-x-auto">
            <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-3xl font-bold text-center mb-2">{c.title}</h2></AnimWrap>
            <AnimWrap {...ea(s,"subtitle")}><p style={subSt} className="text-base opacity-50 text-center mb-10">{c.subtitle}</p></AnimWrap>
        ...
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff" }} className="w-full py-16 px-8">
          ...
          <div className="grid gap-6 max-w-4xl mx-auto" style={{ gridTemplateColumns: `repeat(${(c.plans || []).length}, 1fr)` }}>
            {(c.plans || []).map((p: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <div className={`rounded-2xl p-6 h-full ${p.highlighted ? "bg-purple-600 ring-2 ring-purple-400 shadow-lg shadow-purple-500/20" : "bg-white/6"}`} style={itSt}>
    ...
    case "TESTIMONIALS": {
      ...
      const tCard = (it: any, i: number) => (
        <div key={i} className="bg-white/5 rounded-2xl p-6 border border-white/5">
          ...
          <RH html={it.text} className="text-sm italic mb-4 opacity-80" />
      ...
      if (tv === "quote") {
        ...
        return (
          <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#cbd5e1" }} className="w-full py-24 px-8 flex flex-col items-center text-center">
            ...
            <AnimWrap {...ea(s,"items")}>
              <div className="text-5xl opacity-30 text-purple-400 leading-none mb-2">"</div>
              <RH html={it.text} className="text-2xl italic mb-4 max-w-2xl opacity-80" />
              <div className="text-5xl opacity-30 text-purple-400 leading-none mb-6 self-end">"</div>
            </AnimWrap>
            <div className="flex items-center gap-3">
      ...
      if (tv === "list") return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#cbd5e1" }} className="w-full py-16 px-8">
          <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-3xl font-bold text-center mb-3">{c.title}</h2></AnimWrap>
          {c.subtitle && <AnimWrap {...ea(s,"subtitle")}><p style={subSt} className="text-base opacity-50 text-center mb-10">{c.subtitle}</p></AnimWrap>}
          <div className="max-w-3xl mx-auto space-y-4">
            {(c.items || []).map((it: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>{tCard(it, i)}</AnimWrap>
            ))}
          </div>
        </div>
      );
      return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#cbd5e1" }} className="w-full py-16 px-8">
          <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-3xl font-bold text-center mb-2">{c.title}</h2></AnimWrap>
          {c.subtitle && <AnimWrap {...ea(s,"subtitle")}><p style={subSt} className="text-base opacity-50 text-center mb-8">{c.subtitle}</p></AnimWrap>}
    ...
    case "STATS": {
      ...
      if (stv === "cards") return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff" }} className="w-full py-16 px-8">
          <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-3xl font-bold text-center mb-10">{c.title}</h2></AnimWrap>
          <div className="grid grid-cols-2 gap-6 max-w-2xl mx-auto">
            {(c.items || []).map((it: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <div className="bg-white/5 rounded-2xl p-8 text-center border border-white/5 h-full" style={itSt}>
            ...
            {(c.members || []).map((m: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <div className="flex items-center gap-5 py-4">
                  ...
                  <div className="flex-1" style={itSt}><p className="font-bold">{m.name}</p><p className="text-sm opacity-40">{m.role}</p>{m.bio && <RH html={m.bio} className="text-sm opacity-30 mt-1" />}</div>
                </div>
              </AnimWrap>
            ))}
          </div>
      ...
      return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff" }} className="w-full py-16 px-8">
          ...
          <div className="flex gap-6 justify-center max-w-4xl mx-auto flex-wrap">
            {(c.members || []).map((m: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <div className="bg-white/6 rounded-2xl p-6 text-center flex-1 max-w-[200px]">
                  ...
                  {m.bio && <RH html={m.bio} className="text-xs opacity-30 mt-2 text-left" />}
                </div>
              </AnimWrap>
            ))}
          </div>
      ...
      if (faqv === "grid") return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#e2e8f0" }} className="w-full py-16 px-8">
          ...
          <div className="grid grid-cols-2 gap-5 max-w-4xl mx-auto">
            {(c.items || []).map((it: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <div className="bg-white/4 rounded-2xl p-6 border border-white/5 h-full" style={itSt}>
                  <p className="font-bold mb-2">{it.q}</p>
                  <RH html={it.a} className="text-sm opacity-50" />
                </div>
              </AnimWrap>
            ))}
          </div>
      ...
      return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#e2e8f0" }} className="w-full py-16 px-8">
          ...
          <div className="space-y-3 max-w-2xl mx-auto">
            {(c.items || []).map((it: any, i: number) => (
              <AnimWrap key={i} {...eaItem(s,"items",i)}>
                <details className="border border-white/8 rounded-2xl overflow-hidden group">
                  ...
                  <RH html={it.a} className="px-5 pb-5 text-sm opacity-60 leading-relaxed" />
                </details>
              </AnimWrap>
            ))}
          </div>
    ...
    case "CTA": {
      ...
      if (cv === "banner") return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff" }} className="w-full py-8 px-8">
          <div className="flex items-center justify-between max-w-5xl mx-auto gap-8 flex-wrap">
            <div>
              <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-2xl font-black">{c.title}</h2></AnimWrap>
              <AnimWrap {...ea(s,"subtitle")}><p style={subSt} className="text-sm opacity-60 mt-1">{c.subtitle}</p></AnimWrap>
            </div>
            <AnimWrap {...ea(s,"cta")}>
      ...
      if (cv === "split") return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff" }} className="w-full py-20 px-8 flex items-center max-w-5xl mx-auto gap-12">
          <div className="flex-1">
            <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-4xl font-black mb-3">{c.title}</h2></AnimWrap>
            <AnimWrap {...ea(s,"subtitle")}><p style={subSt} className="text-base opacity-60">{c.subtitle}</p></AnimWrap>
          </div>
          <AnimWrap {...ea(s,"cta")}>
      ...
      return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff" }} className="w-full py-20 px-8 flex flex-col items-center text-center">
          <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-4xl font-black mb-3 max-w-2xl">{c.title}</h2></AnimWrap>
          <AnimWrap {...ea(s,"subtitle")}><p style={subSt} className="text-base opacity-60 mb-8">{c.subtitle}</p></AnimWrap>
          <AnimWrap {...ea(s,"cta")}>
    ...
    case "TEXT":
      return (
        <div style={{ ...bgStyle, color: s.textColor || "#e2e8f0", textAlign: (s.align as any) || "left" }} className="w-full py-16 px-8">
          <div className="max-w-3xl mx-auto">
            {c.title && <AnimWrap {...ea(s,"title")}><h2 style={titleSt} className="text-2xl font-bold mb-4">{c.title}</h2></AnimWrap>}
            <AnimWrap {...ea(s,"body")}>
              {c.bodyHtml ? (
    ...
    case "GALLERY": {
      ...
      if (galv === "masonry") return (
        <div id={anchorId} style={bgStyle} className="w-full py-16 px-8">
          <AnimWrap {...ea(s,"title")}><h2 style={{ color: s.textColor || "#fff", ...titleSt }} className="text-3xl font-bold text-center mb-10">{c.title}</h2></AnimWrap>
      ...
      if (galv === "slider") return (
        <div id={anchorId} style={bgStyle} className="w-full py-16 px-8">
          <AnimWrap {...ea(s,"title")}><h2 style={{ color: s.textColor || "#fff", ...titleSt }} className="text-3xl font-bold text-center mb-10">{c.title}</h2></AnimWrap>
          <AnimWrap {...ea(s,"items")}>
            {allImgs.length > 0
              ? <GallerySlider images={allImgs} s={s} />
              : <div className="max-w-5xl mx-auto rounded-2xl border border-dashed border-white/10 flex items-center justify-center" style={{ aspectRatio: "16/9" }}>
                  <div className="text-center text-white/20">
                    <Image size={40} className="mx-auto mb-2" />
                    <p className="text-sm">Добавьте изображения в настройках</p>
                  </div>
                </div>
            }
          </AnimWrap>
    ...
    case "VIDEO": {
      ...
      if (vidv === "shorts") {
        const shortsVideos = videos.length ? videos : (c.url ? [{ url: c.url }] : []);
        return <ShortsSliderBlock anchorId={anchorId} bgStyle={bgStyle} titleSt={titleSt} s={s} c={c} videos={shortsVideos} />;
      }
      if (vidv === "grid") return (
        <div id={anchorId} style={bgStyle} className="w-full py-16 px-8">
          {c.title && <h2 style={{ color: s.textColor || "#fff", ...titleSt }} className="text-3xl font-bold text-center mb-8">{c.title}</h2>}
          <div className="grid grid-cols-2 gap-6 max-w-4xl mx-auto">
            {(videos.length > 0 ? videos : [{url:""},{url:""},{url:""},{url:""}]).slice(0,4).map((v2: any, i: number) => (
              <div key={i} className="bg-black/50 rounded-2xl overflow-hidden" style={{ aspectRatio: "16/9", position: "relative" }}>
                {v2.url
                  ? <iframe src={toEmbed(v2.url)} className="absolute inset-0 w-full h-full" allowFullScreen allow="autoplay; encrypted-media" />
                  : <div className="absolute inset-0 flex items-center justify-center text-white/20"><Video size={36} /></div>}
              </div>
            ))}
          </div>
          {c.description && <p style={{ color: s.textColor || "#fff" }} className="text-base opacity-60 text-center mt-6 max-w-xl mx-auto">{c.description}</p>}
      ...
      if (vidv === "slider") {
        return <VideoSliderBlock anchorId={anchorId} bgStyle={bgStyle} titleSt={titleSt} s={s} c={c} videos={videos} />;
      }
      return (
        <div id={anchorId} style={bgStyle} className="w-full py-16 px-8">
          <h2 style={{ color: s.textColor || "#fff", ...titleSt }} className="text-3xl font-bold text-center mb-6">{c.title}</h2>
          <div className="max-w-3xl mx-auto bg-black/50 rounded-2xl overflow-hidden" style={{ aspectRatio: "16/9", position: "relative" }}>
            {c.url
              ? <iframe src={toEmbed(c.url)} className="absolute inset-0 w-full h-full" allowFullScreen allow="autoplay; encrypted-media" />
              : <div className="absolute inset-0 flex flex-col items-center justify-center text-white/20 gap-2"><Video size={48} /><p className="text-sm">Вставьте URL видео</p></div>
            }
          </div>
    ...
    case "SHORTS_SLIDER":
      return <ShortsSliderStandaloneBlock anchorId={anchorId} bgStyle={bgStyle} titleSt={titleSt} s={s} c={c} />;
    case "PRODUCTS":
    ...
    case "CONTACTS":
      return (
        <div style={{ ...bgStyle, color: s.textColor || "#e2e8f0" }} className="w-full py-16 px-8">
          <div className="max-w-3xl mx-auto">
            <h2 style={titleSt} className="text-3xl font-bold mb-3">{c.title}</h2>
            <p style={subSt} className="text-base opacity-50 mb-8">{c.subtitle}</p>
    ...
    case "HEADER_MENU": {
      ...
      const menuSt      = elSt(s, "menu");
      const logoSt      = elSt(s, "logo");
      const activeLinkSt = elSt(s, "activeLink");
      const menuHov = elHover(s, "menu");
      const logoHov = elHover(s, "logo");
      const menuDur = ((s.elemStyles || {}).menu || {}).hoverDuration || "0.2s";
      const logoDur = ((s.elemStyles || {}).logo || {}).hoverDuration || "0.2s";
      const navCls  = anchorId ? `hm-${anchorId}` : "hm-nav";
      const hoverCSS = [
        menuHov && `.${navCls}-link { transition: color ${menuDur} ease, opacity ${menuDur} ease; } .${navCls}-link:hover { color: ${menuHov} !important; opacity: 1 !important; }`,
        logoHov && `.${navCls}-logo { transition: color ${logoDur} ease; } .${navCls}-logo:hover { color: ${logoHov} !important; }`,
      ].filter(Boolean).join("\n");
      const activeColor = activeLinkSt.color || "var(--site-primary)";
      const navLinks = (c.links || []).map((l: any, i: number) => (
        <SmartLink key={i} href={l.url || "#"}
          className={`${navCls}-link text-sm ${l.active ? "" : menuSt.color ? "" : "opacity-60 hover:opacity-100"} transition`}
          style={{ ...menuSt, ...(l.active ? { color: activeColor, fontWeight: activeLinkSt.fontWeight || 600 } : {}) }}>
          {l.label}
        </SmartLink>
      ));
      const resolvedCtaBtns: { label: string; href: string; variant: string }[] = c.ctaButtons?.length
        ? c.ctaButtons
        : (c.hideCta || !c.cta) ? [] : [{ label: c.cta, href: c.ctaUrl || "#", variant: "primary" }];
      const ctaBtns = resolvedCtaBtns.map((btn, i) => {
        const isOutline = btn.variant === "outline";
        const isSecondary = btn.variant === "secondary";
        const btnStyle: React.CSSProperties = isOutline
          ? { border: `1.5px solid ${s.ctaColor || "#7c3aed"}`, color: s.ctaColor || "#7c3aed", padding: "6px 20px" }
          : isSecondary
          ? { color: s.textColor || "#e2e8f0", opacity: 0.7, padding: "6px 20px" }
          : { ...ctaStyle(s), padding: "6px 20px" };
        return (
          <SmartLink key={i} href={btn.href || "#"} className="text-sm font-semibold transition flex-shrink-0 rounded-lg" style={btnStyle}>
            {btn.label}
          </SmartLink>
        );
      });
      const cartIcon = s.showCart !== false && <CartNavIcon />;
      const logoEl = c.logoImage
        ? <img src={c.logoImage} alt={c.logo || ""} style={{ height: 36, maxWidth: 160, objectFit: "contain" as const, flexShrink: 0 }} />
        : <span className={`${navCls}-logo text-lg font-bold flex-shrink-0 transition`} style={logoSt}>{c.logo}</span>;
      if (hv === "logo_center") return (
        <>
          ...
          <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#e2e8f0" }} className="w-full sticky top-0 z-40 backdrop-blur-xl">
            <div className="flex items-center justify-center px-8 py-3 border-b border-white/5">{logoEl}</div>
            <div className="flex items-center justify-between px-8 py-2">
              <div className="flex gap-8 flex-1 justify-center">{navLinks}</div>
              <div className="flex items-center gap-2">{cartIcon}{ctaBtns}</div>
            </div>
          </div>
        </>
      );
        ...
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#475569" }} className="w-full py-6 px-8 border-t border-white/5">
          <div className="max-w-5xl mx-auto flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex gap-5 flex-wrap">
              {(c.links || []).map((l: any, i: number) => <SmartLink key={i} href={l.url || "#"} className="text-sm opacity-50 hover:opacity-80 transition">{l.label}</SmartLink>)}
            </div>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex flex-col sm:flex-row justify-between gap-6 mb-8">
              ...
              <div className="flex gap-6 flex-wrap items-start">
                {(c.links || []).map((l: any, i: number) => <SmartLink key={i} href={l.url || "#"} className="text-sm hover:text-white/70 transition">{l.label}</SmartLink>)}
              </div>
            </div>
    ...
    case "SCHEDULE":
      return <ScheduleBlock anchorId={anchorId} bgStyle={bgStyle} titleSt={titleSt} s={s} c={c} />;
    case "COACHES":
      return (
        <div id={anchorId} style={{ ...bgStyle, color: s.textColor || "#fff" }} className="w-full py-16 px-8">
          ...
          <div className="flex gap-6 justify-center max-w-4xl mx-auto">
            {(c.members || []).map((m: any, i: number) => (
              <div key={i} className="bg-white/6 rounded-2xl p-6 text-center flex-1 max-w-[220px]">
                ...
                {m.bio && <RH html={m.bio} className="text-xs opacity-30 mt-2" />}
              </div>
            ))}
          </div>
        </div>
      ...
      const blogCard = (it: any, i: number) => (
        <a key={i} href={it.url || "#"} className="bg-white/5 rounded-2xl overflow-hidden border border-white/5 hover:border-purple-500/30 transition block">
          ...
          <div className="p-5">
            ...
            {it.preview && <RH html={it.preview} className="text-sm opacity-50 mt-2 line-clamp-3" />}
              ...
              {(c.items || []).map((it: any, i: number) => (
                <a key={i} href={it.url || "#"} className="flex gap-5 bg-white/5 rounded-2xl overflow-hidden border border-white/5 hover:border-purple-500/30 transition">
                  ...
                  <div className="py-4 pr-4 flex flex-col justify-center">
                    ...
                    {it.preview && <RH html={it.preview} className="text-sm opacity-50 mt-1 line-clamp-2" />}
    ...
    case "ZERO_BLOCK":
      return (
        <div id={anchorId} className="w-full">
          <ZeroBlockRenderer content={c} styles={s} onOpenPopup={id => popup?.openPopup(id)} />
        </div>
      );
    default:
      return (
...
export default function PreviewPage() {
  ...
  // ─── Derive cart style from HEADER_MENU block (must be before early returns) ─
  const cartStyle = useMemo<CartStyle>(() => {
    const hb = blocks.find(b => b.type === "HEADER_MENU");
    if (!hb) return DEFAULT_CART_STYLE;
    const hs = (() => { try { return JSON.parse(hb.styles || "{}"); } catch { return {}; } })();
    return {
      icon: hs.cartIcon || "bag",
      iconColor: hs.cartIconColor || "",
      badgeColor: hs.cartBadgeColor || "#ec4899",
      bg: hs.cartBg || "#111121",
      accent: hs.cartAccent || "#7c3aed",
      btnText: hs.cartBtnText || "Оформить заказ →",
      showLabel: !!hs.cartShowLabel,
    };
  }, [blocks]);
  ...
  const handlePreviewClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Only intercept relative paths (not external, not anchors, not mailto, not popup)
  ...
  return (
    <CartStyleContext.Provider value={cartStyle}>
    ...
    <div className="min-h-screen bg-[#070711] flex flex-col">
      <div className="sticky top-0 z-50 bg-[#0a0a1a]/90 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 h-12 flex items-center gap-4">
          <button onClick={() => nav(siteId ? `/builder/${siteId}` : "/dashboard")} className="flex items-center gap-1.5 text-white/50 hover:text-white transition text-sm">
            <ArrowLeft size={14} /> В редактор
          </button>
          ...
          <div className="site-preview rounded-xl overflow-hidden shadow-2xl shadow-black/50 border border-white/5"
            ...
            {filteredBlocks.map(block => {
              ...
              const wrapperStyle: React.CSSProperties = {
                ...
                borderWidth: st.borderWidth ? `${st.borderWidth}px` : undefined,
                borderStyle: st.borderWidth ? (st.borderStyle || "solid") : undefined,
                borderColor: st.borderWidth ? (st.borderColor || "rgba(255,255,255,0.15)") : undefined,
                boxShadow: st.boxShadow || undefined,
                borderRadius: st.outerRadius ? `${st.outerRadius}px` : undefined,
                overflow: st.overflow || undefined,
                fontFamily: st.fontFamily || undefined,
                ["--bk-font" as any]: st.fontFamily || undefined,
                ["--bk-title-size" as any]: TITLE_SIZE_MAP[st.titleSize || "xl"] || TITLE_SIZE_MAP.xl,
                ["--bk-fw" as any]: st.fontWeight || "700",
                ["--bk-lh" as any]: st.lineHeight || "1.3",
                ["--bk-ls" as any]: st.letterSpacing !== undefined ? `${st.letterSpacing}px` : "0px",
              ...
              return (
                <div key={block.id} style={wrapperStyle} className="bk-typo">
      ...
      {hasProducts && (() => {
        const accent = cartStyle.accent || "#7c3aed";
        return (
          <button
            onClick={() => setCartOpen(true)}
            className="fixed bottom-6 right-6 z-[100] w-14 h-14 rounded-full transition shadow-lg flex items-center justify-center text-white"
            style={{ backgroundColor: accent, boxShadow: `0 8px 20px ${accent}50` }}
            title="Корзина"
          >
            <CartIconEl name={cartStyle.icon} size={22} />
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full text-white text-[10px] font-bold flex items-center justify-center"
                style={{ backgroundColor: cartStyle.badgeColor }}>
