...
router.post("/storage/uploads/request-url", async (req: Request, res: Response) => {
  const userId = authUser(req, res as any);
  if (!userId) return;
  ...
  try {
    ...
    console.error({ err: error }, "Error generating upload URL");
    res.status(500).json({ error: "Failed to generate upload URL" });
  }
});

// Called by the client after a successful PUT to the signed upload URL.
// Sets owner ACL so the file becomes readable by the uploader.
router.post("/storage/uploads/finalize", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ ok: true, objectPath: normalizedPath });
  } catch (error) {
    if (error instanceof ObjectNotFoundError) {
      res.status(404).json({ error: "Object not found — upload may not have completed yet" });
      return;
    }
    console.error({ err: error }, "Error finalizing upload");
